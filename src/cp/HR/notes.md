...
- Это делается только по явному действию пользователя.
...
## Оптимальные рекомендации по длине логина и пароля следующие:

Логин:
- Минимальная длина обычно составляет от 3 до 7-8 символов, оптимально 8-12 символов. Это обеспечивает удобство и безопасность, так как слишком короткие логины легко угадать, а слишком длинные неудобны для пользователя и интерфейса.

- Максимальная длина логина обычно ограничивается 16-32 символами, чтобы избежать проблем с отображением и обработкой в системе.

- Разрешены буквы (заглавные и строчные), цифры и некоторые специальные символы, например, дефис (-) и подчеркивание (_).

Пароль:
- Минимальная длина пароля должна быть не менее 8 символов, чтобы обеспечить базовую безопасность.

- Оптимальная рекомендуемая длина — от 16 символов и более, так как чем длиннее пароль, тем выше его энтропия и устойчивость к взлому.

- Максимальная длина пароля должна быть не менее 64 символов, а в некоторых системах может доходить до 128 символов, что технически допустимо и повышает безопасность.

- Пароль должен содержать комбинацию заглавных и строчных букв, цифр и специальных символов для повышения сложности.

- Не рекомендуется использовать слишком длинные пароли (более 128 символов), так как они могут обрезаться или не поддерживаться системой.

Таким образом, оптимальные параметры можно сформулировать так:
| Параметр | Минимум      | Оптимум             | Максимум        |
| -------- | ------------ | ------------------- | --------------- |
| Логин    | 3-7 символов | 8-12 символов       | 16-32 символа   |
| Пароль   | 8 символов   | 16 и более символов | 64-128 символов |

Эти рекомендации обеспечивают баланс между удобством пользователя и безопасностью системы.

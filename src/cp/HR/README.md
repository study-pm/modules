## Безопасность
Данный раздел охватывает следующие аспекты:
следующие аспекты:
- механизмы регистрации и защиты данных от несанкционированного доступа;
- механизмы аутентификации и авторизации пользователей (управление ролями пользователей);
- логирование действий пользователя дла аудита и контроля безопасности;
- организация безопасного хранения и передачи данных внутри приложения;
- меры по предотвращению ошибок и сбоев, а также восстановлению данных (резервное копирование).




Управление ролями пользователей и разграничение уровней доступа, чтобы обеспечить, что разные категории пользователей (например, администраторы, учителя, секретари) имеют доступ только к тем функциям и данным, которые им необходимы для работы.

Механизмы аутентификации и авторизации пользователей для предотвращения несанкционированного доступа к персональным данным сотрудников и учащихся.

Защита данных от несанкционированного просмотра, изменения или удаления, включая шифрование данных и резервное копирование.

Логирование действий пользователей для аудита и контроля безопасности, что помогает выявлять и предотвращать злоупотребления.

Обеспечение конфиденциальности и соответствие требованиям законодательства по защите персональных данных.

Организация безопасного хранения и передачи данных внутри приложения, особенно если предусмотрена работа в сетевой среде.

Меры по предотвращению ошибок и сбоев, которые могут привести к потере или искажению данных.
...
- Секрет должен храниться в базе в зашифрованном виде, а не в виде хеша, потому что хеширование — односторонняя операция, и будет невозможно восстановить секрет для генерации кодов.
...
Таким образом, секрет для 2FA нужно хранить в базе в зашифрованном виде, а не в виде хеша с солью. Это обеспечивает баланс между безопасностью и функциональностью проверки одноразовых кодов. Для повышения безопасности следует использовать надёжное шифрование и управление ключами согласно лучшим практикам защиты баз данных. Таким образом, рекомендуется шифровать base32Secret перед сохранением в базе данных. Вот почему:

- base32Secret — это общий секрет, который используется для генерации одноразовых кодов TOTP. Если злоумышленник получит к нему доступ, он сможет подделывать 2FA-коды и обходить защиту.

- Хранение секрета в открытом виде в базе данных увеличивает риск компрометации всей двухфакторной аутентификации при утечке или взломе базы.

- Шифрование секрета (например, с помощью AES) позволяет защитить его, сохраняя возможность расшифровки на сервере для проверки кода.

- Ключ шифрования должен храниться отдельно и надёжно, чтобы свести к минимуму риск одновременного раскрытия секрета и ключа.

- Раздельное хранение пароля и 2FA-секрета повышает безопасность, исключая одновременную компрометацию обоих факторов

| Хранение base32Secret            | Рекомендация     | Причина                                       |
| -------------------------------- | ---------------- | --------------------------------------------- |
| В открытом виде                  | Не рекомендуется | Уязвимость при утечке базы                    |
| В зашифрованном виде (AES и др.) | Рекомендуется    | Защита секрета при хранении                   |
| В виде хеша + соль               | Не подходит      | Нельзя восстановить секрет для генерации TOTP |

Таким образом, шифрование base32Secret — это важная мера безопасности для защиты двухфакторной аутентификации и предотвращения компрометации при утечках данных.

Важные моменты:
- Ключ (Key) и вектор инициализации (IV) должны храниться отдельно от базы, например, в защищённом хранилище, конфигурации с ограниченным доступом или аппаратном модуле безопасности.

- Никогда не стоит хранить ключ и IV вместе с зашифрованными данными в базе.

- Следует использовать длину ключа 32 байта для AES-256.

- IV должен быть 16 байт (128 бит) и может быть фиксированным или случайным (тогда его нужно сохранять вместе с шифротекстом).

- Для упрощения примера здесь IV фиксирован, но в продакшене лучше использовать случайный IV и хранить его вместе с зашифрованными данными.

Таким образом можно безопасно шифровать и хранить base32Secret в базе, обеспечивая защиту двухфакторной аутентификации от компрометации.

Для хранения секретов TOTP в базе данных обычно используют один общий ключ шифрования для всех пользователей, а не уникальный ключ для каждого пользователя. Вот почему:

- Каждый пользователь имеет свой уникальный секрет (base32Secret), который генерируется при настройке 2FA.

- Для защиты этих секретов в базе используют симметричное шифрование (например, AES) с одним ключом, который хранится отдельно и надежно.

- Такой подход упрощает управление ключами, снижает сложность и позволяет централизованно контролировать безопасность.

- При необходимости можно реализовать ротацию ключа шифрования, но это делается централизованно, а не для каждого пользователя индивидуально.

- В редких случаях, когда требуется повышенная безопасность, можно использовать уникальные ключи для пользователей, но это усложняет управление ключами и не является стандартной практикой.

| Вариант                       | Рекомендация        | Комментарии                                             |
| ----------------------------- | ------------------- | ------------------------------------------------------- |
| Один ключ шифрования для всех | Обычно используется | Упрощает управление и хранение ключей                   |
| Уникальный ключ для каждого   | Редко применяется   | Сложнее в управлении, требует инфраструктуры для ключей |

Дополнительно:
- Ключ шифрования должен храниться в безопасном месте (например, в HSM, Azure Key Vault, AWS KMS или защищённом конфигурационном файле с ограниченным доступом).

- При использовании одного ключа важно обеспечить его защиту и аудит доступа.

Таким образом, один надёжный ключ шифрования для всех секретов пользователей — это стандартный и практичный подход для защиты TOTP-секретов в базе данных.
